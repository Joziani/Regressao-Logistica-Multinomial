...
\begin{equation}
g_{k} \equiv g_{k}(\mathbf{x}) = \ln \left[\frac{\mathbb{P}(Y=k \mid \mathbf{x})}{\mathbb{P}(Y=0 \mid \mathbf{x})}\right]
\end{equation}
\begin{equation}
= \beta_{k 0} x_{k 0}+\beta_{k 1} x_{1}+\cdots+\beta_{k r} x_{r}
\end{equation}
\begin{equation}
= x^{\prime} B_{k}, \text { para } k \in\{0, \cdots, q\}
\end{equation}
\begin{equation}
\text {onde } B_{k}=\left(\beta_{k 0}, \cdots, \beta_{k r}\right)^{\prime} \text { e } x_{k 0}=1
\end{equation}
\begin{equation}
\text {e } {r} \text { é a quantidade de variáveis independentes.}
\end{equation}
